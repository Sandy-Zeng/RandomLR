...
# Training parameters
exp_name = '%s_%d_%d_%s_%s_%.2f_%.2f_%.4f_%.2f_ResNet32_Sin' % (dataset_name,epochs,batch_size,optimizer,distribution_method,dis_parameter1,dis_parameter2,linear_init_lr,random_portion)
...
def lr_schedule(epoch):
    ...
    def WRSGN(epoch,tmp_lr,base_lr):
        ...
        if tt >= math.pi:
            tt = tt - math.pi
        curT = t0 + tt
        new_lr = base_lr + tmp_lr * (1.0 + math.sin(curT)) / 2.0  # lr_min = 0, lr_max = lr
    ...
    left = 0
    right = epochs * 0.4
    lr_decay = [linear_init_lr*1e-1,
                linear_init_lr*1e-2,
                linear_init_lr*1e-3,
                linear_init_lr*0.5e-3,
                ]
    base_lr = lr_decay[0]
    if epoch >= epochs * 0.9:
        lr *= 0.5e-3
    ...
    global Te,last_lr
    Te = right - left
    new_lr = WRSGN(epoch, lr,base_lr)
    if new_lr > last_lr and epoch > 0:
        new_lr = last_lr
    else:
        last_lr = new_lr
    lr = new_lr
