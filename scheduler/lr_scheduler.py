...
class Warm_Start_Scheduler(Callback):
    def __init__(self,init_lr=1e-3,Te=10,multFac=2,distribution_method='N',random_range=10,random_potion=0.5,epochs=200):
        ...
        self.Te = Te
        self.t0 = math.pi / 2.0
        self.dt = 2.0 * math.pi / float(2.0 * self.Te)
        self.tt = 0 - self.dt
        self.TeNext = Te
    ...
    def lr_schedule(self,epoch):
        def WRSGN(epoch, tmp_lr):
            ...
            if (epoch + 1 == self.TeNext-1):  # time to restart
                ...
                self.dt = 2.0 * math.pi / float(2.0 * self.Te)
                self.tt = 0-self.dt  # by setting to 0 we set lr to lr_max, see above
        ...
        if self.iscycle:
            ...
            lr = self.last_lr
        if epoch < self.epochs * self.random_potion and epoch>120:
            if self.distribution_method == 'U':
                lr = U(lr, self.random_range)
            if self.distribution_method == 'N':
                lr = N(lr, mu=self.random_range)
