...
def N(tmp_lr, mu=2, sigma=1,random_range=10):
    np.random.seed(int(time.time()))
...
class BatchRLR(Callback):
    ...
    def __init__(self,batches=200,init_lr=1e-3,distribution_method='N',random_potion=0.3,random_range=10):
    ...
    def lr_schedule(self,batch):
        ...
        lr = self.linear_init_lr
        left = 0
        right = self.batches * 0.4
        if batch > self.batches * 0.9:
            lr *= 0.5e-3
            left = self.batches * 0.9
            right = self.batches
        elif batch > self.batches * 0.8:
            lr *= 1e-3
            left = self.batches * 0.8
            right = self.batches * 0.9
        elif batch > self.batches * 0.6:
            lr *= 1e-2
            left = self.batches * 0.6
            right = self.batches * 0.8
        elif batch > self.batches * 0.4:
            lr *= 1e-1
            left = self.batches * 0.4
            right = self.batches * 0.6
